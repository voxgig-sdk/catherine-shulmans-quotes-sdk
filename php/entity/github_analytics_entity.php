...
// CatherineShulmansQuotes SDK GithubAnalytics entity
...
class GithubAnalyticsEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GithubAnalyticsEntity($this->_client, $opts);
    }

    /**
     * @param GithubAnalytics|array $args GithubAnalytics data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CatherineShulmansQuotesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GithubAnalytics|array The current GithubAnalytics data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GithubAnalytics fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CatherineShulmansQuotesHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GithubAnalytics fields).
     */
    ...
    /**
     * Load a single GithubAnalytics.
     *
     * @param GithubAnalyticsLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GithubAnalyticsLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GithubAnalytics|array The loaded GithubAnalytics as an assoc-array at the
     *   SDK boundary; throws CatherineShulmansQuotesError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CatherineShulmansQuotesHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
