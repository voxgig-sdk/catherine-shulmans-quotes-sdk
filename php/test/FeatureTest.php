...
// CatherineShulmansQuotes SDK feature test
//
...
class FtCtrl extends CatherineShulmansQuotesControl
{
...
class FtClient
{
    ...
    private CatherineShulmansQuotesUtility $utility;

    public function __construct(CatherineShulmansQuotesUtility $utility, array $options)
    {
    ...
    public function get_utility(): CatherineShulmansQuotesUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CatherineShulmansQuotesContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CatherineShulmansQuotesContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CatherineShulmansQuotesContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CatherineShulmansQuotesConfig::make_feature($name);
    ...
    private static function buildUrl(CatherineShulmansQuotesSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CatherineShulmansQuotesError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CatherineShulmansQuotesResponse($fetched);
        }
    ...
    private function fail(CatherineShulmansQuotesContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CatherineShulmansQuotesContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
