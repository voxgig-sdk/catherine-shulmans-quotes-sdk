...
class EpisodeEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = CatherineShulmansQuotesSDK::test($seed, null);
        $seen = iterator_to_array($base->Episode(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = CatherineShulmansQuotesConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = CatherineShulmansQuotesSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Episode(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CATHERINESHULMANSQUOTES_TEST_EPISODE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function episode_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("CATHERINESHULMANSQUOTES_TEST_EPISODE_ENTID");
