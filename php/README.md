# CatherineShulmansQuotes PHP SDK



The PHP SDK for the CatherineShulmansQuotes API — an entity-oriented client using PHP conventions.
...
try {
    ...
    print_r($githubanalytics->data_get());
} catch (\Throwable $err) {
    echo "Error: " . $err->getMessage();
}
```
...
print_r(array_map(fn($item) => $item->data_get(), $episode));
```
...
```
php/
├── catherineshulmansquotes_sdk.php          -- Main SDK class
...
The main class (`catherineshulmansquotes_sdk.php`) exports the SDK class
