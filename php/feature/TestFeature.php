...
// CatherineShulmansQuotes SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CatherineShulmansQuotesTestFeature extends CatherineShulmansQuotesBaseFeature
{
    ...
    public function init(CatherineShulmansQuotesContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CatherineShulmansQuotesContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CatherineShulmansQuotesParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CatherineShulmansQuotesContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CatherineShulmansQuotesContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CatherineShulmansQuotesParam::call($ctx, $k);
