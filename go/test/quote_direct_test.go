...
func TestQuoteDirect(t *testing.T) {
	...
	t.Run("direct-load-quote", func(t *testing.T) {
		setup := quoteDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-quote", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func quoteDirectSetup(mockres any) *quoteDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"CATHERINE_SHULMANS_QUOTES_TEST_QUOTE_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CATHERINE_SHULMANS_QUOTES_TEST_QUOTE_ENTID"]; ok {
