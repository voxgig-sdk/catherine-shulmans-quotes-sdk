...
import (
	...
	omni "github.com/voxgig-sdk/catherine-shulmans-quotes-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CatherineShulmansQuotesError); is {
				out := map[string]any{
					"name":    "CatherineShulmansQuotesError",
...
func omniCtx(arg any, client *sdk.CatherineShulmansQuotesSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CatherineShulmansQuotesSDK); is {
			client = live
