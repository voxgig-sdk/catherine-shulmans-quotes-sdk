...
func TestEpisodeEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Episode(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Episode(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CATHERINESHULMANSQUOTES_TEST_EPISODE_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		episodeRef01DataDt0LoadResult := core.ToMapAny(episodeRef01DataDt0Loaded)
...
func episodeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CATHERINESHULMANSQUOTES_TEST_EPISODE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CATHERINESHULMANSQUOTES_TEST_EPISODE_ENTID"])
