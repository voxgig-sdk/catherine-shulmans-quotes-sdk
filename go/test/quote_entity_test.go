...
func TestQuoteEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CATHERINE_SHULMANS_QUOTES_TEST_QUOTE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func quoteBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CATHERINE_SHULMANS_QUOTES_TEST_QUOTE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CATHERINE_SHULMANS_QUOTES_TEST_QUOTE_ENTID"])
