...
func TestGithubAnalyticsEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CATHERINESHULMANSQUOTES_TEST_GITHUB_ANALYTICS_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func github_analyticsBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CATHERINESHULMANSQUOTES_TEST_GITHUB_ANALYTICS_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CATHERINESHULMANSQUOTES_TEST_GITHUB_ANALYTICS_ENTID"])
