...
class GithubLanguageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GithubLanguageEntity(self._client, opts)
    ...
    def data_get(self) -> GithubLanguage:
    ...
    def match_get(self) -> GithubLanguage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> GithubLanguage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GithubLanguage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
