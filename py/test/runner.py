# CatherineShulmansQuotes SDK test runner
...
class CatherineShulmansQuotesTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CatherineShulmansQuotesTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CatherineShulmansQuotesTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CatherineShulmansQuotesTestRunner.getenv("CATHERINE_SHULMANS_QUOTES_TEST_LIVE")
        override = CatherineShulmansQuotesTestRunner.getenv("CATHERINE_SHULMANS_QUOTES_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CatherineShulmansQuotesTestRunner.getenv(key)
        ...
        explain = CatherineShulmansQuotesTestRunner.getenv("CATHERINE_SHULMANS_QUOTES_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CatherineShulmansQuotesTestRunner._test_control is not None:
            return CatherineShulmansQuotesTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CatherineShulmansQuotesTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CatherineShulmansQuotesTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CatherineShulmansQuotesTestRunner.load_test_control()
...
def load_env_local():
    CatherineShulmansQuotesTestRunner.load_env_local()


def env_override(m):
    return CatherineShulmansQuotesTestRunner.env_override(m)


def entity_data(v):
    return CatherineShulmansQuotesTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CatherineShulmansQuotesTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CatherineShulmansQuotesTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CatherineShulmansQuotesTestRunner.load_test_control()


def live_client_options():
    return CatherineShulmansQuotesTestRunner.live_client_options()


def live_delay_ms():
    return CatherineShulmansQuotesTestRunner.live_delay_ms()
