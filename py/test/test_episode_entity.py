...
class TestEpisodeEntity:
    ...
    def test_should_stream(self):
        ...
        base = CatherineShulmansQuotesSDK.test(seed, None)
        seen = list(base.Episode(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = CatherineShulmansQuotesSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Episode(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CATHERINESHULMANSQUOTES_TEST_EPISODE_ENTID JSON to run live")
        client = setup["client"]
...
def _episode_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CATHERINESHULMANSQUOTES_TEST_EPISODE_ENTID"))
