# CatherineShulmansQuotes Lua SDK



The Lua SDK for the CatherineShulmansQuotes API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/catherine-shulmans-quotes-sdk/releases)),
...
```lua
local githubanalytics, err = client:GithubAnalytics():load({ username = "example_username" })
if err then error(err) end
print(githubanalytics)
```
...
Create an instance: `local github_analytics = client:GithubAnalytics(nil)`
...
```lua
local github_analytics, err = client:GithubAnalytics():load({ username = "username" })
```


### GithubCard

Create an instance: `local github_card = client:GithubCard(nil)`
...
```lua
local github_card, err = client:GithubCard():load({ username = "username" })
```


### GithubLanguage

Create an instance: `local github_language = client:GithubLanguage(nil)`
...
```lua
local github_language, err = client:GithubLanguage():load({ username = "username" })
```
...
```
lua/
├── catherine-shulmans-quotes_sdk.lua    -- Main SDK module
...
The main module (`catherine-shulmans-quotes_sdk`) exports the SDK constructor
