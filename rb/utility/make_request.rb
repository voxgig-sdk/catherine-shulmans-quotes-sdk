...
module CatherineShulmansQuotesUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CatherineShulmansQuotesResponse.new({})
    ...
    if fetch_err
      ...
      response = CatherineShulmansQuotesResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CatherineShulmansQuotesResponse.new(fetched)
    else
