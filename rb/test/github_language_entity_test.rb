...
class GithubLanguageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = github_language_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "github_language." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CATHERINE_SHULMANS_QUOTES_TEST_GITHUB_LANGUAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    github_language_ref01_data = nil
    if github_language_ref01_data_raw.length > 0
      github_language_ref01_data = Helpers.to_map(github_language_ref01_data_raw[0][1])
    end

    # LOAD
    github_language_ref01_ent = client.GithubLanguage(nil)
    github_language_ref01_match_dt0 = {}
    github_language_ref01_data_dt0_loaded = github_language_ref01_ent.load(github_language_ref01_match_dt0, nil)
    assert !github_language_ref01_data_dt0_loaded.nil?

  end
end

def github_language_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "github_language", "GithubLanguageTestData.json")
  ...
  entid_env_raw = ENV["CATHERINE_SHULMANS_QUOTES_TEST_GITHUB_LANGUAGE_ENTID"]
