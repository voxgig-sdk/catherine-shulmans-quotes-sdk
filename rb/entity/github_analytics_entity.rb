...
class GithubAnalyticsEntity
  ...
  def make
    opts = @_entopts.dup
    GithubAnalyticsEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CatherineShulmansQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GithubAnalytics, Hash] the current GithubAnalytics data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CatherineShulmansQuotesHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GithubAnalytics fields)
  def match_get
  ...
  #
  # @param reqmatch [GithubAnalyticsLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GithubAnalytics.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GithubAnalytics, Hash] the loaded GithubAnalytics; raises CatherineShulmansQuotesError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CatherineShulmansQuotesHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
