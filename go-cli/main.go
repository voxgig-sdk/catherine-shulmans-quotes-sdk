// Command CatherineShulmansQuotes-cli is a CLI + REPL for the CatherineShulmansQuotes SDK,
...
const prompt = "catherine-shulmans-quotes"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "api episode github_analytics github_card github_language quote"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CATHERINE_SHULMANS_QUOTES_APIKEY (api key), CATHERINE_SHULMANS_QUOTES_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CATHERINE_SHULMANS_QUOTES_APIKEY carries the API key and
	// CATHERINE_SHULMANS_QUOTES_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CATHERINE_SHULMANS_QUOTES_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CATHERINE_SHULMANS_QUOTES_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCatherineShulmansQuotesSDK(opts)
