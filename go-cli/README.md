# catherine-shulmans-quotes-cli

boru-driven command-line client **and** interactive REPL for the CatherineShulmansQuotes
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/catherine-shulmans-quotes-cli)
make build

# 2. See usage (words, entities, env vars)
./catherine-shulmans-quotes-cli --help

# 3. Provide credentials once, via the environment
export CATHERINE_SHULMANS_QUOTES_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./catherine-shulmans-quotes-cli list api
./catherine-shulmans-quotes-cli list episode

# 5. Override the API base URL for a single call
CATHERINE_SHULMANS_QUOTES_BASE=https://api.example.com ./catherine-shulmans-quotes-cli list api

# 6. No arguments -> interactive REPL
./catherine-shulmans-quotes-cli
catherine-shulmans-quotes> list api
catherine-shulmans-quotes> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/catherine-shulmans-quotes-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/catherine-shulmans-quotes-cli list api
   ```
...
Run with no arguments to open a REPL (prompt `catherine-shulmans-quotes>`). Each line is
evaluated as its own boru expression:

```text
$ ./catherine-shulmans-quotes-cli
catherine-shulmans-quotes> list api
catherine-shulmans-quotes> /help
catherine-shulmans-quotes> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/catherine-shulmans-quotes-cli`. |
...
(`./catherine-shulmans-quotes-cli <expr>`) and interactively (the REPL), and expressions compose the
