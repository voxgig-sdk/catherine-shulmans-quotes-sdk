...
module CatherineShulmansQuotesUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = CatherineShulmansQuotesContext.new({}, nil)
    end
    op = ctx.op || CatherineShulmansQuotesOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || CatherineShulmansQuotesResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(CatherineShulmansQuotesError) ? err.msg : err.to_s
    ...
    sdk_err = CatherineShulmansQuotesError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(CatherineShulmansQuotesError)
