...
class CatherineShulmansQuotesMakeRequest
{
    public static function call(CatherineShulmansQuotesContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new CatherineShulmansQuotesResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new CatherineShulmansQuotesResponse($fetched);
        } else {
