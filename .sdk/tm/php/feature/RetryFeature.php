...
// CatherineShulmansQuotes SDK retry feature
...
class CatherineShulmansQuotesRetryFeature extends CatherineShulmansQuotesBaseFeature
{
    ...
    public function init(CatherineShulmansQuotesContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CatherineShulmansQuotesContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CatherineShulmansQuotesContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CatherineShulmansQuotesContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
