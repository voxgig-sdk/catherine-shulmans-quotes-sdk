...
//    only - a typed CatherineShulmansQuotesContext there would make every ctx
...
final class CatherineShulmansQuotesOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CatherineShulmansQuotesContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CatherineShulmansQuotesSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CatherineShulmansQuotesResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CatherineShulmansQuotesError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CatherineShulmansQuotesResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CatherineShulmansQuotesError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CatherineShulmansQuotesError(is_string($code) ? $code : '', $msg);
    }
