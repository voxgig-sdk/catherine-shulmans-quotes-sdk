# catherine-shulmans-quotes-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CatherineShulmansQuotes SDK as
two agent tools — `catherine-shulmans-quotes_list` and `catherine-shulmans-quotes_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/catherine-shulmans-quotes-mcp)
make build

# 2. Provide credentials via the environment
export CATHERINE_SHULMANS_QUOTES_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user catherine-shulmans-quotes \
  -- /absolute/path/to/catherine-shulmans-quotes-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./catherine-shulmans-quotes-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// catherine-shulmans-quotes_list: first page of records
{ "entity": "api" }
{ "entity": "api", "query": { } }

// catherine-shulmans-quotes_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/catherine-shulmans-quotes-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user catherine-shulmans-quotes \
     -- "$PWD"/dist/*/catherine-shulmans-quotes-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `catherine-shulmans-quotes_list` and `catherine-shulmans-quotes_load` tools now appear
   in new sessions. Ask the agent to *"list api using catherine-shulmans-quotes"*
   and it calls `catherine-shulmans-quotes_list` with `{"entity":"api"}`.
...
```sh
export CATHERINE_SHULMANS_QUOTES_APIKEY=sk_live_xxx            # API key
export CATHERINE_SHULMANS_QUOTES_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `catherine-shulmans-quotes_list` tool
...
### Call the `catherine-shulmans-quotes_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `catherine-shulmans-quotes_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `catherine-shulmans-quotes_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CATHERINE_SHULMANS_QUOTES_APIKEY` | API key sent with every request. |
| `CATHERINE_SHULMANS_QUOTES_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

api | episode | github_analytics | github_card | github_language | quote

### Smoke test via HTTP (raw JSON-RPC)

```sh
./catherine-shulmans-quotes-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"catherine-shulmans-quotes_load","arguments":{"entity":"episode","query":{"id":1}}}}'
```
