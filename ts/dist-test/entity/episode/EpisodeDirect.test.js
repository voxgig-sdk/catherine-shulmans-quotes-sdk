...
(0, node_test_1.describe)('EpisodeDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.CatherineShulmansQuotesSDK({
    ...
    (0, node_test_1.test)('direct-load-episode', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-episode', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["episode01", "program01"]))
            return;
        ...
        if (setup.live) {
            ...
            const candidateId = listArr[0]?.episode_id ?? listArr[0]?.id;
            if (null == candidateId) {
                throw new Error('Live load blocked: discovery returned no usable identity');
            }
            params.episode_id = candidateId;
            params.episode_id = setup.idmap['episode01'];
            params.program = setup.idmap['program01'];
        }
        else {
            params.episode_id = 'direct01';
            params.program = 'direct02';
        }
        const result = await client.direct({
            path: 'api/episodes/{program}/{episode_id}',
            method: 'GET',
            params,
            query,
        });
    ...
    (0, node_test_1.test)('direct-list-episode', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-episode', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.CATHERINE_SHULMANS_QUOTES_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.CatherineShulmansQuotesSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['CATHERINE_SHULMANS_QUOTES_TEST_EPISODE_ENTID'];
    ...
    const client = new __1.CatherineShulmansQuotesSDK({
        base: 'http://localhost:8080',
