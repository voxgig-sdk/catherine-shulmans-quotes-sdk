...
const __1 = require("..");
const SDK_NAME = 'CatherineShulmansQuotesSDK';
...
const TEST_SEED = { "entity": { "api": { "test01": { "id": "test01" } }, "episode": { "test01": { "id": "test01" } }, "github_analytics": { "test01": { "id": "test01" } }, "github_card": { "test01": { "id": "test01" } }, "github_language": { "test01": { "id": "test01" } }, "quote": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+CatherineShulmansQuotesSDK\b/.test(code) ||
        /\bCatherineShulmansQuotesSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+CatherineShulmansQuotesSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/CatherineShulmansQuotesSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
