...
describe('GithubAnalyticsEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'github_analytics.' + op, live)) return
    }
    ...
    let github_analytics_ref01_data = Object.values(setup.data.existing.github_analytics)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/github_analytics/GithubAnalyticsTestData.json')
  ...
  const idmapEnvVal = process.env['CATHERINE_SHULMANS_QUOTES_TEST_GITHUB_ANALYTICS_ENTID']
