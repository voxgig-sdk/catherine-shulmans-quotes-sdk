...
describe('EpisodeDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new CatherineShulmansQuotesSDK({
  ...
  test('direct-load-episode', async (t: any) => {
    ...
    if (skipIfMissingIds(t, setup, ["episode01","program01"])) return
  ...
  test('direct-list-episode', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-episode', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CATHERINE_SHULMANS_QUOTES_TEST_EPISODE_ENTID']
