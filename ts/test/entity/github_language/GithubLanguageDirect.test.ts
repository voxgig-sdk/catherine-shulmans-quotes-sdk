...
describe('GithubLanguageDirect', async () => {
  ...
  test('direct-load-github_language', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-github_language', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CATHERINE_SHULMANS_QUOTES_TEST_GITHUB_LANGUAGE_ENTID']
